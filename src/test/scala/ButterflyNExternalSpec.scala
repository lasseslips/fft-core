...
class ButterflyNExternalSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  def testFFTExternalWithSize(n: Int, width: Int, binaryPoint: Int, numTests: Int = 5): Unit = {
    test(new ButterflyNExternal(n, width, binaryPoint)) { dut =>
      ...
      if (PythonFFTVerifier.isPythonAvailable) {
        ...
        for (testNum <- 1 to numTests) {
          println(f"\n--- Random Test Case $testNum/$numTests for ${n}-point FFT External ---")
          ...
          PythonFFTVerifier.verifyNPointFFTWithPython(inputs) match {
            case Some(pythonResult) if pythonResult.length == n =>
              ...
              // Set twiddle factors
              for (i <- twiddleFactorsFixed.indices) {
                val (twReal, twImag) = twiddleFactorsFixed(i)
                dut.io.twiddles(i).real.poke(twReal.S)
                dut.io.twiddles(i).imag.poke(twImag.S)
              }
        ...
        println(f"\nAll $numTests random test cases passed for ${n}-point FFT External!")
  ...
  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 2-point FFT" in {
    testFFTExternalWithSize(2, 16, 8, 5)
  }
  
  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 4-point FFT" in {
    testFFTExternalWithSize(4, 16, 8, 5)
  }
  
  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 8-point FFT" in {
    testFFTExternalWithSize(8, 16, 8, 5)
  }
  
  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 16-point FFT" in {
    testFFTExternalWithSize(16, 32, 16, 3)
  }
}