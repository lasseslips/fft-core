...
class ButterflyNExternalSpec extends AnyFlatSpec with ChiselScalatestTester {
  def genNumStr(nums: Seq[(Double, Double)]): String = {
    nums.map { case (re, im) => f"(${re}%.5f,${im}%.5f)" }.mkString(", ")
  }

  def testFFTExternalWithSize(n: Int, width: Int, binaryPoint: Int, numTests: Int = 5, pipeline: Boolean = false): Unit = {
    test(new ButterflyNExternal(n, width, binaryPoint, pipeline)) { dut =>
      println(s"Testing ${n}-point FFT External against original ButterflyN and Python NumPy FFT")
      ...
      if (PythonFFTVerifier.isPythonAvailable) {
        ...
        val tolerance = 0.01
        val latency = if (pipeline) (math.log(n) / math.log(2)).toInt else 0

        // Generate twiddle factors
        val twiddleFactorsDouble = ButterflyNExternalUtils.generateTwiddleFactors(n)
        val twiddleFactorsFixed = ButterflyNExternalUtils.twiddlesToFixedPoint(twiddleFactorsDouble, width, binaryPoint)
  ...
  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 2-point FFT (non-pipelined)" in {
    testFFTExternalWithSize(2, 16, 8, 5, false)
  }

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 4-point FFT (non-pipelined)" in {
    testFFTExternalWithSize(4, 16, 8, 5, false)
  }

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 8-point FFT (non-pipelined)" in {
    testFFTExternalWithSize(8, 16, 8, 5, false)
  }
  

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 16-point FFT (non-pipelined)" in {
    testFFTExternalWithSize(16, 32, 16, 3, false)
  }

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 2-point FFT (pipelined)" in {
    testFFTExternalWithSize(2, 16, 8, 5, true)
  }

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 4-point FFT (pipelined)" in {
    testFFTExternalWithSize(4, 16, 8, 5, true)
  }

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 8-point FFT (pipelined)" in {
    testFFTExternalWithSize(8, 16, 8, 5, true)
  }

  "ButterflyNExternalSpec" should "match Python NumPy FFT results for 16-point FFT (pipelined)" in {
    testFFTExternalWithSize(16, 32, 16, 3, true)
  }
